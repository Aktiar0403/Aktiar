...
function evaluateCompoundRule(rule, testValues) {
  return rule.conditions.every(cond => {
    const value = testValues[cond.test];
...
export function evaluateRule(rule, visit) {
  if (rule.type === "simple") {
    const value = testValues[rule.test];
    if (value !== undefined) {
      let match = false;
    ...
    if (evaluateCompoundRule(rule, testValues)) results.push(rule);
  }
